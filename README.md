...
- **공개 범위 정책 분리**
  - 접근 권한 판단 로직을 `DiaryPostAccessPolicy`로 분리
  - 본인 → 항상 허용
  - FRIEND → 상호 팔로우 여부 확인
  - PRIVATE → 본인만 조회 가능
- **피드 조회 성능 개선**
  - 작성자·좋아요·댓글·이미지 반복 조회를 JOIN FETCH와 배치 조회로 개선
  - 게시글 수에 비례해 증가하던 반복 조회를 5회 고정 쿼리 구조로 개선
...
> 5인 팀 프로젝트를 개인 포트폴리오용으로 정리한 저장소로, README는 제가 담당한 영역을 중심으로 작성했습니다.
